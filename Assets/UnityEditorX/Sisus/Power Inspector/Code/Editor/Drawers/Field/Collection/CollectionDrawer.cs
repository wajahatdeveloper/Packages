...
namespace Sisus
{
	...
	public abstract class CollectionDrawer<TValue, TSize, TIndex> : ParentFieldDrawer<TValue>, ICollectionDrawer, IReorderableParent where TValue : class, IEnumerable where TSize : struct, IEquatable<TSize> where TIndex : struct, IEquatable<TIndex>
	{
		...
		/// <summary>
		/// If all targets have the same element count, equals said count, else equals -1.
		/// </summary>
		...
		protected override void Setup(TValue setValue, Type setValueType, LinkedMemberInfo setMemberInfo, IParentDrawer setParent, GUIContent setLabel, bool setReadOnly)
		{
			...
			{
				propertyAttributeInfo = new PropertyAttributeInfo(attributes[0] as PropertyAttribute, null);
			}
			#endif
		}
		...
		private LinkedMemberInfo CreateResizerFieldInfo()
		{
			...
			Debug.Assert(memberInfo != null, ToString()+".CreateResizerFieldInfo - memberInfo is required for setting up collection resizer!");
			#endif
		...
		protected sealed override void DoGenerateMemberBuildList()
		{
			...
			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(memberInfo != null, ToString()+".DoGenerateMemberBuildList - memberInfo was null but it is required for CreateResizerFieldInfo!");
			#endif
		...
		protected void UpdateItemCountRect()
		{
			...
			itemCountRect.x = labelLastDrawPosition.xMax - labelSize.x;
		...
		/// </summary>
		/// <param name="newSize"> New cached size value. </param>
		...
		protected void SetCachedSize(TSize setSize, bool mixedContent, bool isNull)
		{
			...
			Debug.Log("SetCachedSize("+setSize+") with mixedContent="+mixedContent+", isNull="+isNull);
			#endif
		...
		public sealed override void UpdateCachedValuesFromFieldsRecursively()
		{
			...
			var sizeWas = size;
			if(!MixedContent && memberInfo != null)
			{
			...
			{
				int n = LastCollectionMemberIndex;
				try
				{
					for(; n >= FirstCollectionMemberIndexOffset; n--)
					{
						members[n].UpdateCachedValuesFromFieldsRecursively();
					}
				}
				catch(NullReferenceException)
				{
					#if DEV_MODE && DEBUG_NULL_MEMBERS
					Debug.LogWarning("UpdateCachedValuesFromFieldsRecursively() NullReferenceException " + ToString()+ " @ members[" + n + "]");
					#endif
				}
				catch(IndexOutOfRangeException)
				{
					#if DEV_MODE
					Debug.LogWarning("UpdateCachedValuesFromFieldsRecursively() IndexOutOfRangeException " + ToString()+ " @ members[" + n + "]");
					#endif
				}
			}
		...
		private void Resize(TSize newSize)
		{
			...
			//size = newSize;
			//elementCount = GetElementCount(newSize);
		...
		[Pure]
		protected abstract TSize GetCollectionSize(TValue collection);
