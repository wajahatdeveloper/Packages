...
namespace Sisus
{
	public sealed class RuntimeGUIDrawer : DrawGUI
	{
		...
		{
			bool clicked = GUI.Button(position, label, InspectorPreferences.Styles.Button) && Event.current.button == 0;
			return clicked;
		}

		public override bool Button(Rect position, GUIContent label, GUIStyle guiStyle)
		{
			bool clicked = GUI.Button(position, label, guiStyle) && Event.current.button == 0;
			return clicked;
		}
