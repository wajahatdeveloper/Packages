...
namespace Sisus
{
	...
	{
		private static readonly Dictionary<Type, Type[]> typesVisibleFromContext = new Dictionary<Type, Type[]>();
		...
		public static Type[] GenerateTypesVisibleFromContext([CanBeNull]Type typeContext, out string[] typeLabels)
		{
			#if DEV_MODE || PROFILE_POWER_INSPECTOR
			...
			if(typeContext != null)
			{
				if(typesVisibleFromContext.TryGetValue(typeContext, out types))
				{
					typeLabels = typesLabelsForTypesVisibleFromContext[typeContext];
				}
				else
				{
					...
					typesVisibleFromContext.Add(typeContext, types);
					typesLabelsForTypesVisibleFromContext.Add(typeContext, typeLabels);
				}

				#if DEV_MODE
				Debug.Log(StringUtils.ToColorizedString("GenerateTypesVisibleFromContext(", typeContext, "): ", types.Length, "\n", StringUtils.ToString(typeLabels, "\n")));
				#endif
			}
			else
			{
				types = TypeExtensions.AllVisibleTypes;
				typeLabels = TypeExtensions.MenuLabelsForAllVisibleTypes;

				#if DEV_MODE
				Debug.Log(StringUtils.ToColorizedString("GenerateTypesVisibleFromContext(", null, "): ", types.Length, "\n", StringUtils.ToString(typeLabels, "\n")));
				#endif
			}
		...
		public static void BuildPopupMenuItemsForEnumType(ref List<PopupMenuItem> results, ref Dictionary<string, PopupMenuItem> groupsByLabel, ref Dictionary<string, PopupMenuItem> itemsByLabel, [NotNull]Type enumType)
		{
			...
			for(int n = 0; n < count; n++)
			{
				...
				BuildPopupMenuItemWithLabel(ref results, ref groupsByLabel, ref itemsByLabel, enumValue, null, enumNames[n], StringUtils.ToString(Convert.ChangeType(enumValue, underlyingType)), null);
			}
		...
		public static void BuildPopupMenuItemForType([NotNull]ref List<PopupMenuItem> results, ref Dictionary<string, PopupMenuItem> groupsByLabel, ref Dictionary<string, PopupMenuItem> itemsByLabel, [NotNull]Type type, string globalNamespaceTypePrefix = "")
		{
			#if DEV_MODE || PROFILE_POWER_INSPECTOR
		...
		[CanBeNull]
		public static string BuildPopupMenuItemForObjectsInChildren([NotNull]ref List<PopupMenuItem> root, [NotNull]ref Dictionary<string, PopupMenuItem> groupsByLabel, [NotNull]ref Dictionary<string, PopupMenuItem> itemsByLabel, [NotNull]GameObject rootGameObject, [NotNull]Type objectType, [CanBeNull]Object select)
		{
			#if DEV_MODE || PROFILE_POWER_INSPECTOR
			...
			if(objectType == Types.GameObject)
			{
				...
				{
					BuildPopupMenuItemForGameObject(ref root, ref groupsByLabel, ref itemsByLabel, gameObjectsInChildren[n], rootTransform);
				}
			...
			if(objectType == Types.Transform || objectType == Types.RectTransform)
			{
				for(int n = 0, gcount = gameObjectsInChildren.Length; n < gcount; n++)
				{
					BuildPopupMenuItemForTransform(ref root, ref groupsByLabel, ref itemsByLabel, gameObjectsInChildren[n].transform, rootTransform);
				}
			...
			if(objectType == Types.UnityObject)
			{
				for(int n = 0, gcount = gameObjectsInChildren.Length; n < gcount; n++)
				{
					BuildPopupMenuItemForGameObjectAndItsComponents(ref root, ref groupsByLabel, ref itemsByLabel, gameObjectsInChildren[n], rootTransform);
				}
			}
			else //either Component or interface type
			{
				...
				for(int n = 0, gcount = gameObjectsInChildren.Length; n < gcount; n++)
				{
					BuildPopupMenuItemForComponentsOnGameObject(ref root, ref groupsByLabel, ref itemsByLabel, gameObjectsInChildren[n], objectType, rootTransform);
				}
			}
		...
		public static void BuildPopupMenuItemForGameObject([NotNull]ref List<PopupMenuItem> results, ref Dictionary<string, PopupMenuItem> groupsByLabel, ref Dictionary<string, PopupMenuItem> itemsByLabel, [NotNull]GameObject gameObject, [NotNull]Transform relativeToRoot)
		{
			#if DEV_MODE || PROFILE_POWER_INSPECTOR
			...
			BuildPopupMenuItemWithLabel(ref results, ref groupsByLabel, ref itemsByLabel, gameObject, hierarchyPath, MenuItemValueType.UnityObject);
		...
		public static void BuildPopupMenuItemForTransform([NotNull]ref List<PopupMenuItem> results, ref Dictionary<string, PopupMenuItem> groupsByLabel, ref Dictionary<string, PopupMenuItem> itemsByLabel, [NotNull]Transform transform, [NotNull]Transform relativeToRoot)
		{
			#if DEV_MODE || PROFILE_POWER_INSPECTOR
			...
			BuildPopupMenuItemWithLabel(ref results, ref groupsByLabel, ref itemsByLabel, transform, hierarchyPath, MenuItemValueType.UnityObject);
		...
		public static void BuildPopupMenuItemForComponentsOnGameObject([NotNull]ref List<PopupMenuItem> results, ref Dictionary<string, PopupMenuItem> groupsByLabel, ref Dictionary<string, PopupMenuItem> itemsByLabel, [NotNull]GameObject gameObject, [NotNull]Type componentType, [NotNull]Transform relativeToRoot)
		{
			#if DEV_MODE || PROFILE_POWER_INSPECTOR
			...
			for(int c = 0, ccount = getComponents.Count; c < ccount; c++)
			{
				...
				if(comp != null)
				{
					BuildPopupMenuItemWithLabel(ref results, ref groupsByLabel, ref itemsByLabel, comp, string.Concat(hierarchyPath, "/", StringUtils.ToStringSansNamespace(comp.GetType())), MenuItemValueType.UnityObject);
				}
			}
			getComponents.Clear();
		...
		public static void BuildPopupMenuItemForGameObjectAndItsComponents([NotNull]ref List<PopupMenuItem> results, ref Dictionary<string, PopupMenuItem> groupsByLabel, ref Dictionary<string, PopupMenuItem> itemsByLabel, [NotNull]GameObject gameObject, [NotNull]Transform relativeToRoot)
		{
			#if DEV_MODE || PROFILE_POWER_INSPECTOR
			...
			string hierarchyPath = relativeToRoot.GetRelativeHierarchyPath(gameObject.transform);
			BuildPopupMenuItemWithLabel(ref results, ref groupsByLabel, ref itemsByLabel, gameObject, string.Concat(hierarchyPath, "/GameObject"), MenuItemValueType.UnityObject);
		...
		public static void BuildPopupMenuItemForGameObject([NotNull]ref List<PopupMenuItem> results, ref Dictionary<string, PopupMenuItem> groupsByLabel, ref Dictionary<string, PopupMenuItem> itemsByLabel, [NotNull]GameObject gameObject)
		{
			#if DEV_MODE || PROFILE_POWER_INSPECTOR
			Profiler.BeginSample("BuildPopupMenuItemForGameObject");
			#endif
			...
			BuildPopupMenuItemWithLabel(ref results, ref groupsByLabel, ref itemsByLabel, gameObject, Types.GameObject, hierarchyPath, "", MenuItemValueType.UnityObject);
		...
		public static void BuildPopupMenuItemForTransform([NotNull]ref List<PopupMenuItem> results, ref Dictionary<string, PopupMenuItem> groupsByLabel, ref Dictionary<string, PopupMenuItem> itemsByLabel, [NotNull]Transform transform)
		{
			#if DEV_MODE || PROFILE_POWER_INSPECTOR
			Profiler.BeginSample("BuildPopupMenuItemForTransform");
			#endif
			...
			BuildPopupMenuItemWithLabel(ref results, ref groupsByLabel, ref itemsByLabel, transform, Types.Transform, hierarchyPath, "", MenuItemValueType.UnityObject);
		...
		public static void BuildPopupMenuItemForGameObjectAndItsComponents([NotNull]ref List<PopupMenuItem> results, ref Dictionary<string, PopupMenuItem> groupsByLabel, ref Dictionary<string, PopupMenuItem> itemsByLabel, [NotNull]GameObject gameObject)
		{
			#if DEV_MODE || PROFILE_POWER_INSPECTOR
			Profiler.BeginSample("BuildPopupMenuItemForGameObjectAndItsComponents");
			#endif
			...
			for(int c = 0, ccount = getComponents.Count; c < ccount; c++)
			{
				...
				if(comp != null)
				{
					BuildPopupMenuItemWithLabel(ref results, ref groupsByLabel, ref itemsByLabel, comp, comp.GetType(), string.Concat(hierarchyPath, "/", StringUtils.ToStringSansNamespace(comp.GetType())), "", MenuItemValueType.UnityObject);
				}
			}
			getComponents.Clear();
		...
		public static void BuildPopupMenuItemWithLabel(ref List<PopupMenuItem> rootItems, ref Dictionary<string, PopupMenuItem> groupsByLabel, ref Dictionary<string, PopupMenuItem> itemsByLabel, [NotNull]object value, string fullMenuName, MenuItemValueType valueType)
		{
			#if DEV_MODE || PROFILE_POWER_INSPECTOR
			Profiler.BeginSample("BuildPopupMenuItemWithLabel");
			#endif

			BuildPopupMenuItemWithLabel(ref rootItems, ref groupsByLabel, ref itemsByLabel, value, value.GetType(), fullMenuName, valueType);
		...
		public static void AddRange(ref List<PopupMenuItem> rootItems, List<PopupMenuItem> addItems)
		{
				...
				for(int r = 0; r < rootCount; r++)
				{
					...
					if(existing.CompareTo(add) == 0)
					{
						...
						if(add.IsGroup)
						{
							...
							AddRange(ref existing.children, add.children);
						}
						break;
					}
				}
