...
namespace Sisus
{
	...
	public class InspectorManager : Singleton<InspectorManager>, IInspectorManager, IDisposable
	{
		...
		[NotNull]
		public TInspector Create<TInspector>(IInspectorDrawer drawer, InspectorPreferences preferences, Object[] inspected, Vector2 scrollPos, bool viewIsLocked) where TInspector : class, IInspector, new()
		{
			TInspector result;
		...
		{
			#if DEV_MODE && PI_ASSERTATIONS
			if(onNextLayoutDelayedSwapTarget.Count > 0) { Debug.LogError("onNextLayoutDelayedSwapTarget not empty: " + StringUtils.ToString(onNextLayoutDelayedSwapTarget)); }
			#endif
