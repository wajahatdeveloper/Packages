//Modifified by Timo Naskali, 2019: Changed the namespace to avoid possible conflicts.

#if !NET_STANDARD_2_0 //doesn't compile with Scripting Runtime Version: .NET 4.x Equivalent and Api Compatibility Level: .NET Standard 2.0 combination for some reason
using System;
using System.Linq;
...
namespace Sisus.Vexe.FastReflection
{
	...
	//public delegate object CtorInvoker();
	public delegate T ParameterlessCtorInvoker<T>();
	...
	public static class FastReflection
    {
        ...
        /// Generates or gets a strongly-typed open-instance delegate to the specified type constructor that takes the specified type params
        /// </summary>
        public static Delegate DelegateForCtor(this Type type)
        {
            int key = kCtorInvokerName.GetHashCode() ^ type.GetHashCode();
            ...
            var dynMethod = new DynamicMethod(kCtorInvokerName, type, Type.EmptyTypes);

            emit.il = dynMethod.GetILGenerator();
            GenCtor(type);

			var delegateType = typeof(ParameterlessCtorInvoker<>).MakeGenericType(type);
			//result = dynMethod.CreateDelegate(typeof(CtorInvoker));
		...
		static void GenCtor(Type type)
        {
            if (type.IsValueType)
            {
                var tmp = emit.declocal(type);
                emit.ldloca(tmp)
                    .initobj(type)
                    .ldloc(0);
            }
            else
            {
				...
				#if DEV_MODE || SAFE_MODE
                if (ctor == null)
                    throw new Exception("Generating constructor for type: " + type + "No empty constructor found!");
				#endif
        ...
        static void GenFieldGetter<TTarget>(FieldInfo field)
        {
            ...
            GenMemberGetter<TTarget>(field, fieldType, field.IsStatic,
                (e, f) =>
                {
                    if (field.IsLiteral)
                    {
                        if (fieldType == typeof(bool))
                            ...
                            throw new NotSupportedException(string.Format("Creating a FieldGetter for type: {0} is unsupported.", fieldType.Name));
                    }
                    else
