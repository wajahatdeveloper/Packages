...
namespace LunarConsolePlugin
{
    ...
    public sealed class LunarConsole : MonoBehaviour
    {
        ...
        IPlatform CreatePlatform(int capacity, int trim, LunarConsoleSettings settings)
        {
            ...
            {
                LunarConsoleNativeMessageCallback callback = NativeMessageCallback;
                return new PlatformAndroid(gameObject.name, callback.Method.Name, LunarConsolePluginInternal.Constants.Version, capacity, trim, GetGestureName(m_gesture), settings);
            }
            #endif
...
namespace LunarConsolePluginInternal
{
    ...
    public static class LunarConsoleAnalytics
    {
        ...
        static LunarConsoleAnalytics()
        {
            ...
            payload.AppendFormat("&cid={0}", UnityWebRequest.EscapeURL(SystemInfo.deviceUniqueIdentifier));
            payload.AppendFormat("&ua={0}", UnityWebRequest.EscapeURL(SystemInfo.operatingSystem));
            payload.AppendFormat("&av={0}", UnityWebRequest.EscapeURL(Constants.Version));
            #if UNITY_EDITOR
            ...
            {
                var productName = UnityWebRequest.EscapeURL(Application.productName);
            ...
            {
                var bundleIdentifier = UnityWebRequest.EscapeURL(identifier);
            ...
            {
                var companyName = UnityWebRequest.EscapeURL(Application.companyName);
        ...
        public static string CreatePayload(string category, string action, int value)
        {
            ...
            payload.AppendFormat("&ec={0}", UnityWebRequest.EscapeURL(category));
            payload.AppendFormat("&ea={0}", UnityWebRequest.EscapeURL(action));
