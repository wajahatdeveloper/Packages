...
namespace SheetCodesEditor
{
    public class LoadedPageData
    {
        ...
        private bool TryLoadStoredDimensions()
        {
            ...
            int[] storedWidth = SheetDataType.Int.GetDataValue(storedWidthDimensions, default, default, true) as int[];
            int[] storedHeights = SheetDataType.Int.GetDataValue(storedHeightDimensions, default, default, true) as int[];
        ...
        private void CalculateDimensions()
        {
            ...
            int identifierWidth = CalculateWidth("Row Name");
            ...
            for (int i = 0; i < sheetPage.columns.Count; i++)
            {
                ...
                int columnWidth = CalculateWidth(column.GetColumnText());
            ...
            for (int i = 0; i < heightSizes.GetLength(1); i++)
            {
                ...
                {
                    int height = heightSizes[j, i] + WindowSettings.HEIGHT_PADDING;
                    heighestHeight = Mathf.Max(heighestHeight, height);
                }

                heightDimensions.Add(heighestHeight);
            }
        }
