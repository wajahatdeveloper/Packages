	...
	public class PropertyDrawer : ParentFieldDrawer<object>
	{
		...
		public override bool DrawPrefix(Rect position)
		{
			...
			if(showGetButton)
			{
				...
				if(GUI.Button(getButtonRect, GetButtonLabel, InspectorPreferences.Styles.MiniButton))
				{
					dirty = true;
					InvokeGet();
			...
			if(ShowSetButton)
			{
				...
				if(GUI.Button(setButtonRect, SetButtonLabel, InspectorPreferences.Styles.MiniButton))
				{
					dirty = true;
					InvokeSet();
					Select(ReasonSelectionChanged.ControlClicked);
