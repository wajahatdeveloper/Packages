...
namespace MalbersAnimations
{
    ...
    public class StepsManager : MonoBehaviour, IAnimatorListener
    {
        ...
        internal void EnterStep(StepTrigger foot)
        {
            ...
            {
                if (Physics.Raycast(foot.transform.position, -transform.up, out footRay, 1, GroundLayer.Value))
                {
                    if (Tracks)
                    {
        ...
        /// <summary>Disable this script, e.g.. deactivate when is sleeping or death </summary>
