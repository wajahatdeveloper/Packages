	...
	public sealed class DrawerGroup : ParentDrawer<object>, IReorderableParent
	{
		...
		public override void SetMembers(IDrawer[] setMembers, bool sendVisibilityChangedEvents = true)
		{
			...
			if(!nowEditingMembers)
			{
				...
				#if DEV_MODE && PI_ASSERTATIONS
				Debug.Assert(inspector.State.filter.HasFilter == hasFilter, "inspector.State.filter.HasFilter="+StringUtils.ToColorizedString(inspector.State.filter.HasFilter)+" != hasFilter="+StringUtils.ToColorizedString(hasFilter));
				#endif
