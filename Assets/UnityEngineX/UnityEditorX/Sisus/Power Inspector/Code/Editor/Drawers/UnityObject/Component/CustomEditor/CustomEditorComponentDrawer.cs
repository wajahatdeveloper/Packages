		...
		private void UpdateIsExpandable()
		{
			...
			else
			{
				...
				else
				{
					try
					{
						expandable = (bool)Editor.GetType().GetMethod("CanBeExpandedViaAFoldout", BindingFlags.Instance | BindingFlags.NonPublic).Invoke(editor, null);
					}
					#if DEV_MODE
					catch(Exception e)
					{
						Debug.LogError("Error getting CanBeExpandedViaAFoldout value for Editor "+(editor == null ? "null" : editor.GetType().Name) + ": "+e);
						expandable = true;
					}
					#else
					catch
